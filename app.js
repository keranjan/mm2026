...
async function loadAllBrackets() {
  try {
    const res = await api(`player_bracket_predictions?select=*`);
    if (!res.ok) return;
    const rows = await res.json();
    rows.forEach(row => {
...
async function loadActualBracket() {
  try {
    const res = await api(`player_bracket_predictions?username=eq.__ACTUAL__&select=*`);
    if (!res.ok) return;
    const rows = await res.json();
    if (rows.length > 0) {
      const row = rows[0];
...
async function saveActualBracket() {
  ...
  const res = await api('player_bracket_predictions?on_conflict=username', {
    method: 'POST',
    prefer: 'resolution=merge-duplicates',
    body: JSON.stringify({
      username: '__ACTUAL__',
...
async function loadBracket() {
  try {
    ...
    const res = await api(`player_bracket_predictions?username=eq.${encodeURIComponent(currentUser)}&select=*`);
    if (!res.ok) return;
    const rows = await res.json();
    if (rows.length > 0) {
      const row = rows[0];
